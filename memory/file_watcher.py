...
class FileWatcher:
    ...
    def mark_internal(self, path: str) -> None:
        """FileWriter 写入前标记，watcher 收到后跳过。"""
        with self._lock:
            self._internal_writes.add(str(Path(path).resolve()))
    ...
    def _dispatch(self, path: Path, event_type: str) -> None:
        rp = str(path.resolve())
        with self._lock:
            if rp in self._internal_writes:
                self._internal_writes.discard(rp)
                return
        parts = path.parts
        if "soul" in parts:
            if path.name in ("SOUL_CORE.md", "SOUL_STYLE.md") and self.on_soul_change:
                self.on_soul_change(path)
            return
        if "profile" in parts:
