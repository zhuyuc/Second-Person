...
WEB_FETCH_TIMEOUT_LARGE_SECONDS = 90
WEB_FETCH_MAX_RESPONSE_BYTES = 8 * 1024 * 1024
WEB_FETCH_MAX_BODY_CHARS = 150_000
# 工具结果溢写：超过此 UTF-8 字节数则落盘，模型侧只保留预览；0 表示关闭
TOOL_SPILL_MAX_INLINE_BYTES = 80_000
TOOL_SPILL_MAX_FILE_BYTES = 20 * 1024 * 1024
TOOL_SPILL_TTL_DAYS = 7
VECTOR_CACHE_MAX_MB = 512
# HNSW 仅在中大规模缓存启用：小库的矩阵乘法更精确且初始化更省，避免
# 为几十/几百条向量引入额外常驻索引开销。
VECTOR_ANN_MIN_VECTORS = 10_000
VECTOR_ANN_CANDIDATE_MULTIPLIER = 5
GRAPH_MAX_NODES = 300
