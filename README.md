...
- **Agent Core**：TurnRuntime 事件循环 → 宿主上下文 → 模型工具决策/执行 → 流式回复
...
1. 宿主读取模型能力、会话历史、项目上下文和记忆检索结果；
2. `PromptAssembler` 先输出静态系统规则，再把本轮动态上下文统一追加到末尾；
3. 压力超过模型窗口配置比例时，生成会话摘要并重建上下文；遇到模型上下文超限且尚未输出时，执行一次强制压缩恢复；
4. 追加可恢复的会话历史和本轮事件消息；
5. 宿主投影当前可用工具 schema，模型决定是否发起工具调用；
6. 本地单用户模式下宿主直接执行工具，并做参数校验、超时、脱敏和注入防护，再将结果作为事件回填下一步骤；
7. 生成最终回复并持久化 turn/event 事实链。

工具规则属于系统提示词固定 block，工具 schema 通过模型请求的 `tools` 参数传递；前端不能指定工具、改变提示词顺序或绕过宿主策略。

当前能力与各文档的权威范围见 `docs/CURRENT_PRODUCT_CAPABILITIES.md`。HTTP/SSE 字段以
`docs/API_CONTRACT.md` 为准，历史方案不作为实现或验收依据。
