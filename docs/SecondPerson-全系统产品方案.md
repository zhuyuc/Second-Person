...
- **对话页 /chat**：默认“高推理”，可选关闭、低、高、最大推理等级；SSE 展示安全进度、工具确认和完整回复，不展示原生隐藏推理；其余交互保持现有规范。
- **记忆中心 /memory** 六 Tab：
...
- 设置和记忆页面由用户主动发起的变更操作走系统确认弹窗并等待真正落库（`wait=True`）后反馈；Agent 的只读工具可直接执行，写入、破坏性和外部副作用工具必须显示范围与参数，获得用户确认后执行并保留审计和结果反馈。
