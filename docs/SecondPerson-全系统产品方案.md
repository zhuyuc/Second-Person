...
> 本文保留为历史产品方案基线；对话编排、提示词和事件契约已演进为当前代码中的事件化 Agent 运行时，本文旧版流程不作为实现依据。
...
- 自动标题：首条消息创建会话后先显示“新对话”，并行调用 LLM 生成 2-8 字标题；模型返回有效标题后自动替换，占位期间不展示用户原话，生成失败则保持“新对话”；
...
1. 配置对话模型并测试连接；其他模型（包括 Embedding）统一在设置页维护；
2. 展示初始 SOUL_CORE 与 SOUL_STYLE，用户可直接编辑确认后落盘，标记完成进入主界面并记录 first_installed 日期。
...
- **对话页 /chat**：默认“高推理”，可选关闭、低、高、最大推理等级；SSE 展示工具进度和完整回复，不展示原生隐藏推理；其余交互保持现有规范。
- **记忆中心 /memory** 六 Tab：
...
- 设置和记忆页面由用户主动发起的变更操作走系统确认弹窗并等待真正落库（`wait=True`）后反馈；Agent 的所有工具直接执行，工具事件（执行中/结果）在对话流中展示。
...
| 检索 | 用户可拧：top_k 10、向量阈值 0.55、兜底 0.35、去重严格度 enum（loose/normal/strict）；其它（BM25 下限、rrf_k、来源权重等）收敛到 memory/_constants.py |
| 输出画像 | 提炼间隔 7 天、保留 90 天、自动演化 on；其它内部参数（批阈值、窗口天数）收敛到 memory/_constants.py |
| 可视化 | 图谱 300 节点 / 2000 边 |
| 其他 | 会话排队 3、常规回答表达密度、深度回答质量校验 on、抓取超时 15s、IM 单条 4000 字、向量缓存 512MB、浏览器定位 off、mimo 内置搜索 on（关键词上限 3） |
...
5. **问题解决闭环**：推理等级 → 模型工具决策 → 宿主策略执行/确认 → 结果事件回填 → 下一步模型判断 → 用户反馈校准。
