...
import { importApi, uploadDocumentStream } from '@/api/imports'
...
async function loadList() {
  ...
  stats.value = d.stats
  try {
    domains.value = await memoryApi.domains()
  } catch {
    /* 静默 */
  }
...
async function openDetail(id, sug = null) {
  detailSug.value = sug
  detail.value = await memoryApi.detail(id)
}
const detailSug = ref(null)
async function acceptDetailSug() {
  const sug = detailSug.value
  detail.value = null
  detailSug.value = null
...
async function archive(id) {
  ...
  detail.value = null
  toast.push('success', '已归档')
}
async function restore(id) {
  await memoryApi.restore(id)
  await refreshMemoryViews()
  detail.value = null
  toast.push('success', '已恢复为活跃')
}
async function del(id) {
  if (!(await confirm.ask({ message: '永久删除该记忆及全部引用？不可恢复。', danger: true })))
    {return}
...
async function saveAttr(field, value) {
  await memoryApi.updateAttributes(detail.value.id, { [field]: value })
  await openDetail(detail.value.id)
  await loadList()
}

async function loadTimeline() {
  timeline.value = await memoryApi.timeline(tlDays.value, tlType.value || undefined)
}
...
async function loadProfile() {
  try {
    const [p, s, o] = await Promise.all([
      ...
      soulApi.outputStyle(),
      (async () => {
        soulHistory.value = await soulApi.styleHistory(soulSource.value)
      })(),
      soulApi
        .pending()
        .then((p) => {
...
async function loadRespStrategy() {
  try {
    const [r, q] = await Promise.all([
      soulApi.responseStrategy(),
      soulApi.profileReviewPending('strategy_preference'),
    ])
...
async function toggleAutoEvolve() {
  ...
  await soulApi.toggleOutputStyleAuto(target)
  outputStyle.value = await soulApi.outputStyle()
...
async function switchSoulSource(s) {
  soulSource.value = s
...
async function showDiff(v) {
  diffData.value = await soulApi.styleDiff(soulSource.value, v - 1, v)
}
async function rollback(v) {
  if (
    !(await confirm.ask({
      message: `回滚 ${soulSource.value} 序列到 v${v}？只影响该序列负责的段落。`,
    }))
  )
    {return}
  await soulApi.styleRollback(soulSource.value, v)
  await loadProfile()
  toast.push('success', '已回滚')
}
async function confirmPending(pid, approved) {
  await soulApi.confirmPending(pid, approved)
  pendings.value = await soulApi.pending()
...
async function loadGovernance() {
  governance.value = await memoryApi.governance()
}
async function loadCandidates() {
  candidates.value = await memoryApi.candidates()
}
async function confirmCandidate(id) {
  await memoryApi.confirmCandidate(id)
  toast.push('success', '候选已确认，达到门槛后写入长期记忆')
  await loadCandidates()
}
async function rejectCandidate(id) {
  await memoryApi.rejectCandidate(id, '用户在记忆中心拒绝')
  toast.push('success', '已拒绝，后续不再重复推荐')
  await loadCandidates()
}
async function resolveGovernance(item, action = 'reviewed') {
  await memoryApi.resolveGovernance(item.item_id, action)
...
async function openDupCompare(sug) {
  const [a, b] = await Promise.all([
    memoryApi.detail(sug.memory_a.id),
    memoryApi.detail(sug.memory_b.id),
  ])
...
async function resolveDup(sug, res) {
  const labels = {
    keep_a: '已保留 A，删除 B',
    keep_b: '已保留 B，删除 A',
    keep_both: '已确认非重复，两条均保留',
    delete_both: '已删除两条重复记忆',
  }
  if (
    res !== 'keep_both' &&
    !(await confirm.ask({
    ...
    {return}
  await memoryApi.resolveDuplicate(sug.id || sug, res)
...
async function acceptSug(id) {
  const r = await memoryApi.acceptSuggestion(id)
  // 采纳 = 确认内容正确并保留，必定成功；建链只是附带动作，无相似记忆时先以独立节点保留
  if (r && r.linked === false)
    {toast.push('success', '已采纳保留。暂无相似记忆可关联，图谱中先以独立节点存在，后续会自动补链')}
  else toast.push('success', '已采纳')
  await refreshMemoryViews()
}
async function dismissSug(id) {
  await memoryApi.dismissSuggestion(id, 'not_duplicate')
  await refreshMemoryViews()
}
async function confirmLowConf(memory_id) {
  await memoryApi.updateAttributes(memory_id, { confidence: 'medium' })
  await refreshMemoryViews()
  toast.push('success', '已确认，置信度升级为中')
}
async function resolveConflict(cid, res) {
  await memoryApi.resolveConflict(cid, res)
...
async function runLint() {
  ...
  try {
    await memoryApi.runLint()
    await refreshMemoryViews()
    const cur = health.value?.health_score
    toast.push(
      'success',
      cur === prev ? `检查完成，健康度 ${cur} 分（无变化）` : `检查完成，健康度 ${prev} → ${cur} 分`
    )
  } finally {
...
async function saveOutputStyle() {
  if (
    !(await confirm.ask({
      title: '修改输出样式画像',
      message: '将覆盖当前画像内容（自动演化开启时后续仍可能自动更新）。确认保存？',
    }))
  )
    {return}
  await soulApi.updateOutputStyle(outputStyleDraft.value)
  showOutputStyleEdit.value = false
  await loadProfile()
  toast.push('success', '已保存，下次会话生效')
}
...
function openSoulCoreEdit() {
  soulCoreDraft.value = soul.value.soul_core || ''
  showSoulCoreEdit.value = true
}
async function saveSoulCore() {
  if (
    !(await confirm.ask({
      title: '修改核心人格',
      message: 'SOUL_CORE 是 AI 核心人格，修改会影响全局行为。确认保存？',
    }))
  )
    {return}
  await soulApi.updateSoulCore(soulCoreDraft.value)
...
async function resetSoulCore() {
  if (
    !(await confirm.ask({
      title: '恢复默认人格',
      message: '将恢复 SOUL_CORE 为系统默认基线，当前自定义人格将丢失。确认？',
      danger: true,
    }))
  )
    {return}
  await soulApi.resetSoulCore()
  await loadProfile()
  toast.push('success', '已恢复默认人格')
}

const conflicts = ref([])
const conflictCompare = ref(null) // { conflict_id, loading, detailA, detailB }
async function loadConflicts() {
  conflicts.value = await memoryApi.conflicts()
}
async function openConflictCompare(cf) {
  conflictCompare.value = {
  ...
  try {
    const [a, b] = await Promise.all([
      cf.source_a?.memory_id ? memoryApi.detail(cf.source_a.memory_id) : Promise.resolve(null),
      cf.source_b?.memory_id ? memoryApi.detail(cf.source_b.memory_id) : Promise.resolve(null),
    ])
    state.detailA = a
    state.detailB = b
  } finally {
...
async function loadDocs() {
  docs.value = await importApi.documents()
}
async function openDocDetail(id) {
  docDetail.value = await importApi.document(id)
}
function triggerDocPick() {
  docFileInput.value && docFileInput.value.click()
}
...
async function uploadDocs(fileList) {
  ...
  try {
    ...
    for (let fi = 0; fi < files.length; fi++) {
      ...
      if (errored || !result) {
        fail++
        docQueue.value[fi].status = 'failed'
        continue
      }
      ok++
...
function openNextPreview() {
  importPreview.value = previewQueue.value.shift() || null
}
...
async function submitImportPreview(discardAll = false) {
  ...
  try {
    const selected = discardAll ? [] : p.items.filter((_, i) => p.checked[i]).map((it) => it.index)
    const r = await importApi.confirmDocument(p.doc_id, selected)
    toast.push(
      'success',
      `已写入 ${r.written} 条记忆${r.discarded ? `，丢弃 ${r.discarded} 条` : ''}`
    )
...
async function deleteDoc(d) {
  ...
  const res = await importApi.deleteDocument(d.id, cascade)
  await loadDocs()
  if (cascade) {
    toast.push(
...
async function loadLocalDirs() {
  localDirs.value = await importApi.localDirs()
}
async function addLocalDir() {
  const p = localDirPath.value.trim()
  if (!p) {
    toast.push('warning', '请输入目录路径')
    return
  }
  try {
    const d = await importApi.addLocalDir(p, localDirRecursive.value)
...
async function removeLocalDir(dir) {
  ...
  if (!r) return
  await importApi.deleteLocalDir(dir.id)
  toast.push('success', '已移除，已导入内容不受影响')
  await loadLocalDirs()
}
async function toggleLocalDir(dir) {
  await importApi.updateLocalDir(dir.id, { enabled: !dir.enabled })
...
async function scanLocalDirs() {
  ...
  try {
    const r = await importApi.scanLocalDirs()
    ...
    localDirScanning.value = false
  }
}
async function openLocalDirFiles(dir) {
  localDirFiles.value = { dir, files: await importApi.localDirFiles(dir.id) }
}
...
<template>
  ...
  <div v-else-if="tab === 1">
    <div class="g4 mb-3">
      <div class="card">
        ...
        <div class="val">
          {{ stats.total_active + stats.total_stable + stats.total_stale || 0 }}
        </div>
      ...
      <div class="card">
        ...
        <div class="val val-succ">{{ stats.health_score || 0 }}</div>
      </div>
    </div>
    <div class="filter-bar">
      ...
      <select v-model="domainFilter" @change="loadList">
        ...
        <option v-for="d in domains" :key="d.domain" :value="d.domain">
          {{ d.label || domainLabel(d.domain) }}（{{ d.count }}）
        </option>
      </select>
      ...
      <label class="fg check-label">
        <input v-model="importantOnly" type="checkbox" @change="loadList" /> 只看重要记忆
      </label>
    </div>
    <div v-if="!memList.length" class="empty">
      <i class="ti ti-brain"></i>还没有记忆<br />对话中系统会自动沉淀记忆
    </div>
    <div v-for="m in memList" :key="m.id" class="cw list-row" @click="openDetail(m.id)">
      <div class="row">
        <div>
          ...
          <div v-if="m.lifecycle === 'missing'" class="list-sub list-sub-warn">
            <i class="ti ti-alert-triangle"></i> 文件丢失，可从备份恢复文件后自动重建，或删除此索引
          </div>
        </div>
        <div class="fg">
          <span class="badge badge-fade">{{ domainLabel(m.domain) }}</span>
          <span v-if="m.is_important" class="badge badge-g">重要</span>
          <span class="badge badge-a">{{ CONF_MAP[m.confidence] || m.confidence }}</span>
          <span class="badge" :class="m.lifecycle === 'missing' ? 'badge-r' : ''">{{
            LIFE_MAP[m.lifecycle] || m.lifecycle
          }}</span>
  ...
  <div v-else-if="tab === 2">
    <div class="filter-bar">
      <button
        class="chip"
        :class="{ active: tlType === '' }"
        @click="tlType = ''; loadTimeline()"
      >
        全部
      </button>
      <button
        v-for="(label, key) in EVT_MAP"
        :key="key"
        class="chip"
        :class="{ active: tlType === key }"
        @click="tlType = key; loadTimeline()"
      >
        {{ label }}
      </button>
      <select v-model="tlDays" class="ml-auto" @change="loadTimeline">
    ...
    <div v-if="!timeline.length" class="empty">
      <i class="ti ti-timeline"></i>还没有变更记录<br />记忆的新建、更新、合并等操作将显示在这里
    </div>
      ...
      <div v-for="(t, i) in g.items" :key="i" class="cw list-row" @click="openDetail(t.memory_id)">
        ...
        <div class="row mt-1">
          ...
          <button
            ...
            @click.stop="openDetail(tlLinkTarget(t))"
          >
            <i class="ti ti-link"></i> 查看关联
          </button>
  ...
  <div v-else-if="tab === 3">
    ...
    <div v-if="!profile.dimensions.length" class="empty">
      <i class="ti ti-user"></i>用户画像积累中<br />系统在观察你的偏好
    </div>
    <div class="g2">
      <div
        v-for="(d, i) in profile.dimensions"
        :key="i"
        class="cw list-row"
        @click="openDimDetail(d)"
      >
        <div class="dim-header">
          <b>{{ d.name }}</b
          ><span class="badge badge-g">{{ dimStatusLabel(d.status) }}</span>
        </div>
    ...
    <div v-if="pendings.length" class="cw card-warn">
      ...
      <div v-for="p in pendings" :key="p.id" class="pending-item">
        ...
        <div class="fg fg-gap-6 mt-2">
          <button
            class="btn-xs"
            :disabled="busy('pend' + p.id)"
            @click="run('pend' + p.id, () => confirmPending(p.id, true))"
          >
            确认
          </button>
          <button
            class="btn-xs"
            :disabled="busy('pend' + p.id)"
            @click="run('pend' + p.id, () => confirmPending(p.id, false))"
          >
            忽略
          </button>
        </div>
    ...
    <div class="cw">
      <div class="row">
        ...
        <div class="fg fg-gap-6">
          ...
          <button
            class="btn-sm"
            :disabled="busy('resetSoul')"
            @click="run('resetSoul', resetSoulCore)"
          >
            <i v-if="busy('resetSoul')" class="ti ti-loader-2"></i> 恢复默认
          </button>
        </div>
      </div>
      <pre class="mt-2 pre-wrap">{{ soul.soul_core }}</pre>
    </div>
    <div class="cw">
      <div class="tabs mb-3">
        <button
          class="tab"
          :class="{ active: soulSource === 'dialog' }"
          @click="switchSoulSource('dialog')"
        >
          对话确认
        </button>
        <button
          class="tab"
          :class="{ active: soulSource === 'auto' }"
          @click="switchSoulSource('auto')"
        >
          自动演化
        </button>
      </div>
      <div class="mb-3">
        <div v-for="s in currentStyleSections" :key="s.name" class="mb-3">
          ...
          <pre v-if="s.text" class="style-body">{{ s.text }}</pre>
      ...
      <div v-for="h in soulHistory" :key="h.version" class="version-row">
        <div>
          <b>v{{ h.version }}</b> <span v-if="h.current" class="badge badge-a">当前</span>
        ...
        <div class="fg fg-gap-6">
          ...
          <button
            v-if="!h.current"
            class="btn-xs"
            :disabled="busy('rb' + h.version)"
            @click="run('rb' + h.version, () => rollback(h.version))"
          >
            回滚
          </button>
        </div>
      ...
      <div class="row">
        ...
        <div class="fg fg-gap-8">
          <label class="fg check-label">
            <input
              type="checkbox"
              :checked="outputStyle.auto_evolve_enabled"
              @change="toggleAutoEvolve"
            />
            自动演化
          </label>
          <button
            class="btn-sm"
            :disabled="busy('buildOut')"
            @click="run('buildOut', buildOutputStyle)"
          >
            <i v-if="busy('buildOut')" class="ti ti-loader-2"></i> 手动提炼
          </button>
          <button class="btn-sm" @click="openOutputStyleEdit">
            <i class="ti ti-edit"></i> 编辑
          </button>
        </div>
      </div>
      <div class="mt-2 text-sec">
        {{
          outputStyle.profile_text ||
          '（积累中，信号数 ' + (outputStyle.signal_count || 0) + '/50）'
        }}
      </div>
      <div class="muted mt-3 text-muted-sm">
        已采集 {{ outputStyle.signal_count || 0 }} 条信号
        <template v-if="outputStyle.last_built">
          · 上次提炼 {{ formatTimeFull(outputStyle.last_built) }}</template
        >
        <template v-if="outputStyle.is_cold_start"> · 首次提炼需满 50 条</template>
        <template v-else>
          · 新增满 {{ outputStyle.batch_threshold || 100 }} 条或到达周期自动提炼</template
        >
        <template v-if="!outputStyle.auto_evolve_enabled">
          · <span class="text-warn">自动演化已暂停（仍在采集信号）</span></template
        >
      </div>
    </div>
    <div class="sep"></div>
    ...
    <div v-if="respStrategy.candidates.length" class="cw card-warn">
      ...
      <div v-for="p in respStrategy.candidates" :key="p.id" class="pending-item">
        <div class="pending-proposal pending-title">{{ p.title }}</div>
        <div class="pending-trigger mt-1">建议：{{ p.proposed_content }}</div>
        <div class="fg fg-gap-6 mt-2">
          <button
            class="btn-xs"
            :disabled="busy('sc' + p.id)"
            @click="run('sc' + p.id, () => confirmStrategyCand(p.id))"
          >
            确认
          </button>
          <button
            class="btn-xs"
            :disabled="busy('sc' + p.id)"
            @click="run('sc' + p.id, () => rejectStrategyCand(p.id))"
          >
            拒绝
          </button>
        </div>
    ...
    <div class="cw">
      ...
      <pre v-if="respStrategy.content" class="mt-2 pre-wrap">{{ respStrategy.content }}</pre>
      <div v-else class="muted mt-2">
        暂无已确认偏好，系统正从你的赞踩反馈中积累（期间以行业通用默认策略兜底）
      </div>
    </div>
  </div>
  ...
  <div v-else-if="tab === 4">
    <div v-if="health" class="g4 mb-4">
      <div class="card text-center">
        <svg width="84" height="84" viewBox="0 0 84 84" class="health-chart">
        ...
        <button class="btn-sm mt-2" :disabled="lintRunning" @click="runLint">
          <i v-if="lintRunning" class="ti ti-loader-2"></i>
          {{ lintRunning ? '检查中…' : '立即检查' }}
        </button>
        <div
          v-if="
            health.health_score < 100 && (health.score_breakdown || []).some((b) => b.deduct > 0)
          "
      ...
      <div class="card">
        ...
        <div class="val">
          {{ health.stats.pending_confirm }} /
          <span class="text-warn">{{ health.stats.disputed }}</span>
        </div>
      </div>
      <div class="card">
    ...
    <div v-if="conflicts.length" class="cw">
      <div class="cw-heading">
        矛盾记忆 <span class="muted">{{ conflicts.length }} 条待处理</span>
      </div>
      <div v-for="cf in conflicts" :key="cf.conflict_id" class="cw mt-2">
        <div class="list-row" @click="openConflictCompare(cf)">
          ...
          <span class="muted compare-hint"><i class="ti ti-columns"></i> 查看对比</span>
        </div>
      </div>
    </div>
    <!-- Lint 检查明细 -->
    <div v-if="health" class="cw">
      ...
      <div
        v-for="chk in health.lint_details.filter((c) => !c.actionable)"
        :key="chk.check"
        class="version-row"
      >
        <span
          >{{ chk.check }} <span class="muted">· {{ chk.desc }}</span></span
        >
        <span
          class="badge"
          :class="[
            chk.status === 'ok' ? 'badge-g' : '',
            chk.status === 'warning' && chk.count ? 'badge-y' : '',
          ]"
          >{{ chk.count }}</span
        >
      </div>
    ...
    <template v-if="health">
      ...
      >
      <div class="cw-heading">
        {{ chk.check }} <span class="muted">{{ chk.desc }}（{{ chk.count }}）</span>
      </div>
      <div v-for="sug in chk.suggestion_ids || []" :key="sug.id || sug" class="lint-sug-row">
        <span class="lint-sug-label">
          ...
          <template v-else-if="chk.check === '重复检测'"
            >{{ sug.memory_a?.title || sug.memory_a?.id }} ↔
        ...
        <div class="lint-sug-actions">
          <button
            v-if="chk.check === '孤立检测'"
            class="btn-xs"
            @click="openDetail(sug.memory_id, sug)"
          >
            <i class="ti ti-eye"></i> 查看
          </button>
          <template v-if="chk.check === '重复检测'">
            <button class="btn-xs" @click="openDupCompare(sug)">
              <i class="ti ti-columns"></i> 对比查看
            </button>
          </template>
          <template v-else-if="chk.check === '低置信度未确认'">
            <button class="btn-xs" @click="openDetail(sug.memory_id)">
              <i class="ti ti-eye"></i> 查看
            </button>
            <button
              class="btn-xs btn-primary"
              :disabled="busy('low' + sug.memory_id)"
              @click="run('low' + sug.memory_id, () => confirmLowConf(sug.memory_id))"
            >
              确认
            </button>
          </template>
          <template v-else>
            <button
              class="btn-xs"
              :disabled="busy('sug' + (sug.id || sug))"
              @click="run('sug' + (sug.id || sug), () => acceptSug(sug.id || sug))"
            >
              采纳
            </button>
            <button
              class="btn-xs"
              :disabled="busy('sug' + (sug.id || sug))"
              @click="run('sug' + (sug.id || sug), () => dismissSug(sug.id || sug))"
            >
              忽略
            </button>
          </template>
  ...
  <div v-else-if="tab === 5">
    <div class="cw mb-4">
      <div class="section-row mb-2">
        <span class="section-sub section-sub-reset">
          <i class="ti ti-folder icon-accent"></i>本地目录
        </span>
      ...
      <div class="filter-bar filter-bar-mb0">
        <input
          v-model="localDirPath"
          placeholder="输入本地目录绝对路径，如 D:\Documents"
          class="flex-1"
          @keyup.enter="addLocalDir"
        />
        <label class="fg fg-gap-4 check-label shrink-0">
          <input v-model="localDirRecursive" type="checkbox" /> 包含子目录
        </label>
      ...
      <div v-for="dir in localDirs" :key="dir.id" class="dir-row">
        <div class="min-w-0-flex">
          <div class="doc-name">
            {{ dir.path }}
            <span class="badge" :class="dir.enabled ? 'badge-g' : ''">{{
              dir.enabled ? '启用' : '已暂停'
            }}</span>
        ...
        <div class="fg fg-gap-6 local-actions">
          <button class="btn-xs" @click="openLocalDirFiles(dir)">
            <i class="ti ti-list"></i> 文件
          </button>
          <button class="btn-xs" @click="toggleLocalDir(dir)">
            {{ dir.enabled ? '暂停' : '启用' }}
          </button>
          <button class="btn-xs btn-danger" @click="removeLocalDir(dir)">
            <i class="ti ti-trash"></i>
          </button>
        </div>
      </div>
      <div v-if="!localDirs.length" class="muted empty-local">尚未接入本地目录</div>
    </div>
    <div
      class="doc-drop"
    ...
    >
      ...
      <template v-if="docProgress">
        <div class="list-title mt-2">
          正在导入「{{ docProgress.filename }}」<template v-if="docProgress.totalFiles > 1"
            >（{{ docProgress.index }}/{{ docProgress.totalFiles }}）</template
          >
        </div>
        ...
        <template v-if="docProgress.totalFiles > 1">
          <div class="progress-row">
            ...
            <span class="muted progress-pct">{{ docOverallPct }}%</span>
          </div>
          <div class="doc-queue">
        ...
        <div
          v-else-if="docProgress.stage === 'distilling' && docProgress.total"
          class="progress progress progress-narrow"
        >
          <div
            :style="{
              width: (docProgress.current / docProgress.total) * 100 + '%',
      ...
      <template v-else>
        ...
        <div class="muted">
          支持 PDF / DOCX / TXT / MD / 图片（PNG、JPG 等，经 VLM/OCR
          提取），自动解析并提炼为知识库记忆
        </div>
      </template>
      <input ref="docFileInput" type="file" multiple class="hidden-input" @change="onDocPick" />
    </div>
    <div v-if="!docs.length" class="empty mt-4">
      <i class="ti ti-files"></i>知识库还没有文档<br />上传文档后系统会自动解析并提炼记忆
    </div>
    <div v-for="d in docs" :key="d.id" class="cw doc-row mt-3">
      <div class="min-w-0-flex">
        <div class="doc-name">
          <i class="ti ti-file-text icon-accent"></i>{{ d.filename }}
        </div>
        <div class="doc-meta">
          {{ fmtSize(d.size) }} · 提炼 {{ d.memory_count }} 条记忆 ·
          {{ formatTimeFull(d.imported_at) }}
        </div>
      </div>
      <div class="fg fg-gap-6 local-actions">
        <button
          class="btn-sm"
          :disabled="busy('docV' + d.id)"
          @click="run('docV' + d.id, () => openDocDetail(d.id))"
        >
          <i class="ti ti-eye"></i> 查看
        </button>
        <button class="btn-sm btn-danger" @click="deleteDoc(d)">
  ...
  <div v-else-if="tab === 6">
    ...
    <div v-if="!governance.length" class="empty">
      <i class="ti ti-circle-check"></i>暂无待处理事项<br />记忆状态保持良好
    </div>
      ...
      <div class="row">
        <div class="min-w-0-flex">
          ...
          <div class="muted mt-1">
            优先级 {{ Math.round(g.priority || 0) }} · {{ formatTimeFull(g.created_at) }}
          </div>
        </div>
        <div class="fg fg-gap-6 local-actions">
          <button class="btn-xs" @click="openDetail(g.primary_memory_id)">
            <i class="ti ti-eye"></i> 查看
          </button>
  ...
  <div v-else-if="tab === 7">
    ...
    <div v-if="!candidates.length" class="empty">
      <i class="ti ti-circle-check"></i>暂无待确认候选
    </div>
    <div v-for="c in candidates" :key="c.candidate_id" class="cw list-row">
      <div class="row">
        <div class="min-w-0-flex">
          ...
          <div class="muted mt-1">
            评分 {{ Math.round(c.score || 0) }} · 证据 {{ c.evidence_count || 0 }} 条 · 到期
            {{ c.expires_at || '-' }}
          </div>
          <div v-if="c.decision_reason" class="muted mt-1">{{ c.decision_reason }}</div>
        </div>
        <div class="fg fg-gap-6 local-actions">
          <button class="btn-xs btn-primary" @click="confirmCandidate(c.candidate_id)">
            <i class="ti ti-check"></i> 确认保存
          </button>
          <button class="btn-xs" @click="rejectCandidate(c.candidate_id)">
  ...
  <BaseModal v-if="detail" title="记忆详情" stacked @close="detail = null">
    ...
    <!-- missing：文件丢失提示 -->
    <div
      v-if="detail.degraded || detail.frontmatter?.lifecycle === 'missing'"
      class="banner banner banner-warn-mb"
    >
      <i class="ti ti-alert-triangle"></i> 该记忆的 md
      文件丢失。可从备份恢复文件后自动重建索引，或直接删除此索引。
    </div>
    ...
    <dl class="kv">
      ...
      <template v-if="detail.frontmatter?.created_by">
        ...
        <dd>
          {{
            detail.frontmatter.created_by === 'user_explicit'
              ? '用户主动'
              : detail.frontmatter.created_by === 'import'
                ? '导入'
                : '提炼引擎'
          }}
        </dd>
    ...
    <p class="mb-3 text-sec">{{ detail.summary }}</p>
    <div class="label">详情</div>
    <p class="mb-3 detail-body-scroll">{{ detail.detail }}</p>
    <div class="filter-bar mb-3">
      <select
        :value="detail.frontmatter?.confidence"
        @change="(e) => saveAttr('confidence', e.target.value)"
      >
      ...
      <select
        v-if="
          detail.frontmatter?.lifecycle !== 'archived' &&
          detail.frontmatter?.lifecycle !== 'missing'
        "
        :value="detail.frontmatter?.lifecycle"
        @change="(e) => saveAttr('lifecycle', e.target.value)"
      >
        <option value="active">活跃</option>
        <option value="stable">稳定</option>
        <option value="stale">过期</option>
      </select>
      <label class="fg fg-gap-4 check-label">
        <input
          type="checkbox"
          :checked="!!detail.frontmatter?.is_important"
          @change="(e) => saveAttr('is_important', e.target.checked)"
        />
        重要记忆
      </label>
    </div>
    <div v-if="detail.linked_memories && detail.linked_memories.length">
      ...
      <div class="fg fg-gap-6 mb-3 fg-wrap">
        <span
          v-for="lk in detail.linked_memories"
          :key="lk.id"
          class="badge list-row"
          @click="openDetail(lk.id)"
          >{{ lk.type }} · {{ lk.title }}</span
        >
      </div>
    </div>
    <div v-if="detail.change_history && detail.change_history.length">
      <div class="label">变更历史（{{ detail.change_history.length }}）</div>
      <div class="history-scroll modal-scroll-sm">
        <div
          v-for="(h, hi) in detail.change_history"
          :key="hi"
          class="citation-row fs-base"
        >
          {{ h }}
        </div>
      </div>
    ...
    <div v-if="detail.citations && detail.citations.length">
      ...
      <div class="history-scroll">
        <div v-for="(ct, ci) in detail.citations" :key="ci" class="citation-row">
          <span class="muted shrink-0">{{ formatTimeFull(ct.cited_at) }}</span>
          <span class="citation-title" :title="ct.session_title">{{ ct.session_title }}</span>
          <button class="btn-xs shrink-0" @click="jumpToSession(ct.session_id)">
            <i class="ti ti-message"></i> 查看对话
          </button>
        </div>
      </div>
    </div>
    <div class="action-footer">
      ...
      <template v-if="detailSug">
        ...
        <button
          class="btn-primary"
          :disabled="busy('accSug')"
          @click="run('accSug', acceptDetailSug)"
        >
          <i v-if="busy('accSug')" class="ti ti-loader-2"></i> 采纳
        </button>
      </template>
      <button
        v-if="detail.frontmatter?.lifecycle === 'archived'"
        :disabled="busy('mAct')"
        @click="run('mAct', () => restore(detail.id))"
      >
        <i class="ti ti-restore"></i> 恢复
      </button>
      <button
        v-else-if="detail.frontmatter?.lifecycle !== 'missing' && !detail.degraded"
        :disabled="busy('mAct')"
        @click="run('mAct', () => archive(detail.id))"
      >
        归档
      </button>
      <button class="btn-danger" @click="del(detail.id)">
        {{ detail.degraded || detail.frontmatter?.lifecycle === 'missing' ? '删除索引' : '删除' }}
      </button>
    </div>
  </BaseModal>
  <BaseModal
  ...
  >
    <div class="muted mb-3">
      「{{ importPreview.filename }}」提炼出
      {{ importPreview.items.length }} 条候选记忆，勾选需要写入的条目（未勾选的将丢弃）：
    </div>
    <div class="modal-scroll-md mb-3">
      <label v-for="(it, i) in importPreview.items" :key="i" class="fg fg import-row">
        <input v-model="importPreview.checked[i]" type="checkbox" class="import-check" />
        <span class="flex-1">
          <b>{{ it.title }}</b>
          <span class="badge badge-ml-6">{{
            ATTR_MAP[it.attribution] || it.attribution
          }}</span>
          <span class="badge badge-a badge-ml-4">{{
            CONF_MAP[it.confidence] || it.confidence
          }}</span>
          <span class="muted import-summary">{{ it.summary }}</span>
        </span>
      </label>
      <div v-if="!importPreview.items.length" class="empty empty-pad-20">
        本文档未提炼出可入库的内容
      </div>
    </div>
    <div class="action-footer">
      <button class="dang" :disabled="previewSubmitting" @click="submitImportPreview(true)">
        全部丢弃
      </button>
      <button class="btn-primary" :disabled="previewSubmitting" @click="submitImportPreview(false)">
        写入勾选的 {{ previewCheckedCount() }} 条
      </button>
    </div>
  </BaseModal>
  <BaseModal v-if="diffData" title="版本对比" @close="diffData = null">
    <div class="label">旧版</div>
    <pre class="pre-block pre-block-sm">{{ diffData.from }}</pre>
    <div class="label mt-3">新版</div>
    <pre class="pre-block pre-block-sm">{{ diffData.to }}</pre>
    <template #footer>
      <button @click="diffData = null">关闭</button>
    </template>
  </BaseModal>
  <BaseModal
    v-if="showSoulCoreEdit"
    title="编辑 SOUL_CORE 核心人格"
    @close="showSoulCoreEdit = false"
  >
    <div class="muted mb-2 text-warn">
      ⚠ 核心人格影响全局行为，保存前会二次确认，下次会话生效。
    </div>
    <textarea v-model="soulCoreDraft" class="editor-area"></textarea>
    <template #footer>
      <button @click="showSoulCoreEdit = false">取消</button>
      <button
        class="btn-primary"
        :disabled="busy('saveSoul')"
        @click="run('saveSoul', saveSoulCore)"
      >
        <i v-if="busy('saveSoul')" class="ti ti-loader-2"></i> 保存
      </button>
    </template>
  </BaseModal>
  <BaseModal
    v-if="showOutputStyleEdit"
    title="编辑输出样式画像"
    @close="showOutputStyleEdit = false"
  >
    <div class="muted mb-2 text-warn">
      ⚠ 将覆盖当前画像内容，保存前会二次确认，下次会话生效。
    </div>
    <textarea v-model="outputStyleDraft" class="editor-area"></textarea>
    <template #footer>
      <button @click="showOutputStyleEdit = false">取消</button>
      <button
        class="btn-primary"
        :disabled="busy('saveOut')"
        @click="run('saveOut', saveOutputStyle)"
      >
        <i v-if="busy('saveOut')" class="ti ti-loader-2"></i> 保存
      </button>
    </template>
  </BaseModal>
  <BaseModal v-if="docDetail" title="文档详情" @close="docDetail = null">
    <h3 class="modal-subtitle">
      <i class="ti ti-file-text icon-accent"></i>{{ docDetail.filename }}
    </h3>
    <div class="muted mb-3">
      {{ fmtSize(docDetail.size) }} · 提炼 {{ docDetail.memory_count }} 条记忆 ·
      {{ formatTimeFull(docDetail.imported_at) }}
    </div>
    ...
    <div
      v-for="m in docDetail.memories"
      :key="m.id"
      class="cw list-row p-3"
      @click="openDetail(m.id)"
    >
      <b>{{ m.title }}</b>
      <div class="muted">{{ m.summary }}</div>
    </div>
    <div v-if="!docDetail.memories.length" class="empty empty-pad-doc">该文档还没有提炼记忆</div>
    <div v-if="docDetail.citations && docDetail.citations.length">
      ...
      <div class="history-scroll">
        <div v-for="(ct, ci) in docDetail.citations" :key="ci" class="citation-row">
          <span class="muted shrink-0">{{ formatTimeFull(ct.cited_at) }}</span>
          <span class="citation-title" :title="ct.memory_title + ' · ' + ct.session_title"
            >{{ ct.memory_title }} · {{ ct.session_title }}</span
          >
          <button class="btn-xs shrink-0" @click="jumpToSession(ct.session_id)">
            <i class="ti ti-message"></i> 查看对话
          </button>
  ...
  <BaseModal v-if="localDirFiles" title="目录文件" @close="localDirFiles = null">
    ...
    <div class="muted mb-3">
      已导入 {{ localDirFiles.files.filter((f) => f.status === 'imported').length }}/{{
        localDirFiles.files.length
      }}
      个文件
    </div>
    <div class="modal-scroll-md mb-3">
      <div v-for="(f, i) in localDirFiles.files" :key="i" class="version-row fs-base">
        <span class="citation-title" :title="f.path">{{ f.path }}</span>
        <span
          class="badge"
          :class="f.status === 'imported' ? 'badge-g' : f.status === 'failed' ? 'badge-r' : ''"
          >{{ LOCAL_FILE_STATUS[f.status] || f.status }}</span
        >
        <div v-if="f.fail_reason" class="muted mt-1 fs-xs w-full-block">{{ f.fail_reason }}</div>
      </div>
      <div v-if="!localDirFiles.files.length" class="empty empty-pad-20">
        还没有文件记录<br />接入后扫描即可生成
      </div>
    </div>
    <div class="action-footer">
  ...
  <BaseModal v-if="dupCompare" title="疑似重复对比" size="xl" @close="dupCompare = null">
    <div class="action-footer mb-3 footer-mt0">
      <button
        class="btn-xs"
        :disabled="busy('dup')"
        @click="run('dup', () => resolveDup(dupCompare.sug, 'keep_a'))"
      >
        保留 A
      </button>
      <button
        class="btn-xs"
        :disabled="busy('dup')"
        @click="run('dup', () => resolveDup(dupCompare.sug, 'keep_b'))"
      >
        保留 B
      </button>
      <button
        class="btn-xs btn-primary"
        :disabled="busy('dup')"
        @click="run('dup', () => resolveDup(dupCompare.sug, 'keep_both'))"
      >
        都保留
      </button>
      <button
        ...
        :disabled="busy('dup')"
        @click="run('dup', () => resolveDup(dupCompare.sug, 'delete_both'))"
      >
        全部删除
      </button>
    </div>
    <div class="g2 compare-grid">
      <div v-for="(m, k) in [dupCompare.a, dupCompare.b]" :key="k" class="cw compare-col">
        <div class="row mb-2">
          <b :style="{ color: k === 0 ? 'var(--acctx)' : 'var(--warntx)' }"
            >记忆 {{ k === 0 ? 'A' : 'B' }}</b
          >
          <button
            v-if="m"
            class="btn-xs"
            @click.stop="openDetail(m.id); dupCompare = null"
          >
        ...
        <template v-if="m">
          ...
          <div class="compare-meta">
            <span class="badge">{{
              CONF_MAP[m.frontmatter?.confidence] || m.frontmatter?.confidence
            }}</span>
            <span class="badge">{{
              LIFE_MAP[m.frontmatter?.lifecycle] || m.frontmatter?.lifecycle
            }}</span>
            <span v-if="m.frontmatter?.domain" class="badge">{{
              domainLabel(m.frontmatter.domain)
            }}</span>
            <span v-if="m.frontmatter?.source_type" class="badge">{{
              SRC_MAP[m.frontmatter.source_type] || m.frontmatter.source_type
            }}</span>
            <span class="muted">{{ m.frontmatter?.created_at }}</span>
            <span class="muted">引用 {{ m.access_count || 0 }} 次</span>
          </div>
          <p class="compare-body">{{ [m.summary, m.detail].filter(Boolean).join('\n\n') }}</p>
        </template>
        <div v-else class="muted compare-empty">记忆不存在</div>
      </div>
    </div>
    <div class="action-footer">
  ...
  <BaseModal v-if="conflictCompare" title="矛盾记忆对比" size="xl" @close="conflictCompare = null">
    <div class="action-footer mb-3 footer-mt0">
      <button
        class="btn-xs"
        :disabled="busy('conf')"
        @click="run('conf', () => resolveConflict(conflictCompare.conflict_id, 'keep_a'))"
      >
        保留 A
      </button>
      <button
        class="btn-xs"
        :disabled="busy('conf')"
        @click="run('conf', () => resolveConflict(conflictCompare.conflict_id, 'keep_b'))"
      >
        保留 B
      </button>
      <button
        class="btn-xs btn-primary"
        :disabled="busy('conf')"
        @click="run('conf', () => resolveConflict(conflictCompare.conflict_id, 'keep_both'))"
      >
        都保留
      </button>
      <button
        class="btn-xs btn-danger"
        :disabled="busy('conf')"
        @click="run('conf', () => resolveConflict(conflictCompare.conflict_id, 'delete_both'))"
      >
        全部删除
      </button>
    </div>
    <div v-if="conflictCompare.detectedAt" class="muted mb-3">
      检测于 {{ conflictCompare.detectedAt }}
    </div>
    <div v-if="conflictCompare.loading" class="muted compare-loading">加载中…</div>
    <template v-else>
      <div class="g2 compare-grid">
        <div v-for="(side, k) in ['A', 'B']" :key="k" class="cw compare-col">
          <div class="row mb-2">
            <b :style="{ color: k === 0 ? 'var(--acctx)' : 'var(--warntx)' }">记忆 {{ side }}</b>
            <button
              v-if="k === 0 ? conflictCompare.detailA : conflictCompare.detailB"
              class="btn-xs"
          ...
          <template v-if="k === 0 ? conflictCompare.detailA : conflictCompare.detailB">
            <div class="list-title mb-2">
              {{
                (k === 0 ? conflictCompare.detailA : conflictCompare.detailB).frontmatter?.title ||
                (k === 0 ? conflictCompare.detailA : conflictCompare.detailB).id
              }}
            </div>
            <div class="compare-meta">
              <span class="badge">{{
                CONF_MAP[
                  (k === 0 ? conflictCompare.detailA : conflictCompare.detailB).frontmatter
                    ?.confidence
                ] ||
                (k === 0 ? conflictCompare.detailA : conflictCompare.detailB).frontmatter
                  ?.confidence
              ...
              <span class="badge">{{
                ...
                (k === 0 ? conflictCompare.detailA : conflictCompare.detailB).frontmatter?.lifecycle
              }}</span>
              <span
                v-if="
                  (k === 0 ? conflictCompare.detailA : conflictCompare.detailB).frontmatter?.domain
                "
                class="badge"
                >{{
                  domainLabel(
              ...
              <span class="muted"
                >引用
                {{
                  (k === 0 ? conflictCompare.detailA : conflictCompare.detailB).access_count || 0
                }}
                次</span
              >
            </div>
            <p class="compare-body">
              {{
                [
                  (k === 0 ? conflictCompare.detailA : conflictCompare.detailB).summary,
                  (k === 0 ? conflictCompare.detailA : conflictCompare.detailB).detail,
                ]
                  .filter(Boolean)
                  .join('\n\n')
              }}
          ...
          <div v-else class="muted compare-empty">
            {{
              (k === 0 ? conflictCompare.sourceA : conflictCompare.sourceB)?.content || '记忆不存在'
            }}
          </div>
        </div>
      </div>
  ...
  <BaseModal v-if="dimDetail" @close="dimDetail = null">
    <template #header>
      {{ dimDetail.name }}
      <span class="badge badge-g badge-ml-8">{{ dimStatusLabel(dimDetail.status) }}</span>
    </template>
    <div v-for="(it, j) in dimDetail.items" :key="j" class="mb-3 dim-detail-body">
      · {{ it.text }} <span v-if="it.inferred" class="muted">[推断]</span>
    </div>
    <div v-if="!dimDetail.items || !dimDetail.items.length" class="empty empty-pad-doc">
      该维度还没有内容
    </div>
    <template #footer>
