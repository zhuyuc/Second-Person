...
<template>
    <div class="dr-block">
        <!-- 空状态 -->
        <div v-if="isEmpty()" class="dr-empty">
            <i class="ti ti-chart-dots"></i> 图表数据为空
        </div>

        <!-- 按类型分发 -->
        <FlowChartSVG v-else-if="type === 'flowchart'" :nodes="data.nodes || []" :edges="data.edges || []"
            @node-click="onNodeClick" />

        <MermaidChart v-else-if="type === 'mermaid'" :diagram_type="data.diagram_type || 'flowchart'"
            :code="data.mermaid_code || ''" />

        <!-- 未知类型 -->
        <div v-else class="dr-error">
            <i class="ti ti-alert-circle"></i> 未知图表类型：{{ type }}
        </div>
    </div>
</template>
