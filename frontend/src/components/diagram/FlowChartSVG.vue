...
const edgeRanks = computed(() => {
  ...
  props.edges.forEach((e) => {
    ...
    result[`${e.from}|${e.to}`] = fromRank !== null && fromRank !== undefined && toRank !== null && toRank !== undefined && toRank <= fromRank
  })
  return result
})
...
<template>
  <div class="fc-wrap">
    ...
    <div class="fc-actions">
      <button class="fc-btn" title="放大" @click="zoomCenter(view.step)">
        <i class="ti ti-zoom-in"></i>
      </button>
      <button class="fc-btn" title="缩小" @click="zoomCenter(1 / view.step)">
        <i class="ti ti-zoom-out"></i>
      </button>
      <button class="fc-btn" title="复位视图（也可双击画布）" @click="view.reset()">
        <i class="ti ti-focus-2"></i>
      </button>
      <button class="fc-btn" title="复制 SVG 源码" @click="copySvg">
        <i class="ti ti-copy"></i> SVG
      </button>
      <button class="fc-btn" title="下载 SVG" @click="downloadSvg">
        <i class="ti ti-download"></i> SVG
      </button>
      <button class="fc-btn" title="下载 PNG" @click="downloadPng">
        <i class="ti ti-photo"></i> PNG
      </button>
    </div>
    ...
    >
      ...
      <g class="fc-viewport" :transform="fcTransform">
        ...
        <g v-for="e in edges" :key="'e_' + e.from + '_' + e.to">
          <path
            :class="['fc-edge', { focused: isEdgeFocused(e), 'fc-back-edge': isBackEdge(e) }]"
            :d="getEdgePath(e)"
            fill="none"
          ...
          />
          <rect
            v-if="e.label && edgeLabelPos(e).show"
            :x="edgeLabelPos(e).x - 14"
            :y="edgeLabelPos(e).y - 11"
            width="28"
            height="14"
            rx="4"
            fill="var(--surface)"
        ...
        >
          ...
          <rect
            v-if="nodeShape(n).rect"
            :x="nodeShape(n).rect.x"
            :y="nodeShape(n).rect.y"
            :width="nodeShape(n).rect.w"
            :height="nodeShape(n).rect.h"
