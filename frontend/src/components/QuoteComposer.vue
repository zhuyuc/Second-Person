...
const isLong = computed(() => previewLines.value.length > PREVIEW_LINES
    || (props.quoteText || '').length > 240)
...
function onConfirm() { emit('confirm', { comment: comment.value.trim() }) }
...
<template>
    <BaseModal v-if="visible" title="添加引用评论" size="sm" @close="onCancel">
        <div class="quote-composer-preview" :class="{ collapsed: !expanded && isLong }">
            <pre>{{ previewText }}</pre>
            <button v-if="isLong" type="button" class="quote-expand-btn"
                @click="expanded = !expanded">{{ expanded ? '收起' : '展开全部' }}</button>
        </div>
        <label class="quote-composer-label">评论（可选）</label>
        <textarea ref="inputEl" v-model="comment" rows="3" class="quote-composer-input"
            placeholder="对这段引用说点什么？留空也可以直接确认" @keydown="onKeydown"></textarea>
        <template #footer>
