...
<template>
    <div class="handoff-attach">
        <i class="ti"
            :class="status === 'generating' ? 'ti-loader-2' : (status === 'ready' ? 'ti-file-text' : 'ti-alert-triangle')"
            :style="{ color: status === 'failed' ? 'var(--dangtx)' : status === 'ready' ? 'var(--succtx)' : 'var(--muted)' }"></i>
        ...
        <span v-else class="handoff-label dang">
            摘要生成失败
        </span>
        <button v-if="status === 'ready'" type="button" class="handoff-btn" aria-label="预览上一会话摘要" @click="emit('preview')">
            <i class="ti ti-eye"></i> 预览
        </button>
        <button v-if="status !== 'generating'" type="button" class="handoff-btn" aria-label="移除上一会话摘要" @click="emit('remove')">
...
.handoff-label { flex: 1; min-width: 0; }
.handoff-btn {
...
.handoff-btn:hover { background: var(--surface-2); }
</style>
