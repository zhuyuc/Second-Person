...
const MODES = computed(() => [
  { value: 'read-only', label: '只读', icon: 'ti-lock',
    desc: '模型只能读文件；写/编辑/shell 全拒' },
  { value: 'workspace-write', label: props.hasProject ? '项目可写' : '工作区可写',
    icon: 'ti-pencil',
    desc: props.hasProject
      ? '模型可读写项目根内文件；shell 拒'
      : '模型可读写 data/workspace/ 内文件；shell 拒' },
  { value: 'danger-full-access', label: '⚠ 全盘可写', icon: 'ti-alert-triangle',
...
async function load() {
  if (!props.sessionId) { mode.value = props.fallbackMode; source.value = null; return }
  try {
...
watch(() => props.fallbackMode, () => { if (!props.sessionId) mode.value = props.fallbackMode })
onMounted(load)
...
<template>
  <div v-if="mode" class="sandbox-chip"
    ...
    <div v-if="menuOpen" class="chip-menu" @click.stop>
      <div v-for="m in MODES" :key="m.value" class="menu-item"
...
.sandbox-chip:hover { background: var(--bg-hover, rgba(127,127,127,0.08)); }
.sandbox-chip.is-danger { color: var(--dangtx, #c02020); }
.sandbox-chip.is-readonly { color: var(--muted); opacity: 0.7; }
.chip-label { color: var(--fg); font-weight: 500; }
.sandbox-chip.is-danger .chip-label { color: var(--dangtx); }
.chip-caret { padding: 0 2px; }
.chip-menu {
...
.menu-item:hover { background: var(--bg-hover, rgba(127,127,127,0.08)); }
.menu-item.active { background: var(--acctx-bg, rgba(60,120,220,0.15)); }
.menu-item .ti { padding-top: 2px; }
.menu-label { font-weight: 500; margin-bottom: 2px; color: var(--fg); }
.menu-desc { font-size: 12px; color: var(--muted); line-height: 1.4; }
.chip-mask { position: fixed; inset: 0; z-index: 99; }
</style>
