...
function openSearch() { searchOpen.value = true; expandSection('sess') }
...
function projectSwitcherKey(e) {
  ...
  if (e.key === 'ArrowDown') {
    ...
    projectSwitcherHi.value = Math.min(
      projectSwitcherHi.value + 1, projectSwitcherList.value.length - 1)
...
async function archiveManual(sid) {
  ...
  if (!ok) return
  try {
    await api.post('/chat/session/archive', { session_id: sid })
...
const workspaceProjects = computed(() => {
  ...
  return projStore.list.map(p => {
    const list = (byProject[p.id] || []).sort(
    ...
    if (sess.pendingProjectId === p.id && !sess.currentSid) {
      list.unshift({
        ...
        title: '新对话',
        pinned: false, readonly: false, channel: null,
        project_id: p.id, archived: false, is_pending: true,
      })
    }
...
function openAddProject() { addProjOpen.value = true }
...
function toggleProjMenu(pid, ev) {
  if (projMenuId.value === pid) { projMenuId.value = null; return }
...
function closeProjMenu() { projMenuId.value = null }
...
async function saveRenameProject(p) {
  ...
  if (t && t !== p.title) {
    try { await projStore.rename(p.id, t) } catch { /* toast 已弹 */ }
  }
}
...
<template>
  <div class="side-panel">
    ...
    <!-- 工作区（v5 新增） -->
    <div v-if="!searchOpen" class="side-workspace"
      ...
      <div v-for="p in workspaceProjects" :key="p.id" class="ws-project">
        <div class="ws-project-row" @click="toggleWs(p.id)">
          ...
          <input v-if="renamingProjId === p.id" :id="'proj-rename-' + p.id"
            v-model="renameProjTitle" class="sess-rename-input"
            @click.stop @blur="saveRenameProject(p)"
        ...
        <div v-if="!wsCollapsed[p.id]" class="ws-sessions">
          <div v-for="s in p.sessions" :key="s.session_id"
               class="sess-item"
            ...
            <div v-if="menuId === s.session_id" class="sess-menu"
              ...
              <div @click="archiveManual(s.session_id)"><i class="ti ti-archive"></i> 归档会话</div>
              <div class="dang" @click="deleteSession(s.session_id)"><i class="ti ti-trash"></i> 删除</div>
            </div>
          </div>
          <div v-if="!p.sessions.length" class="ws-sessions-empty">
            点上方 ⋯ → 新建会话
          </div>
        </div>
        <div v-if="projMenuId === p.id" class="proj-menu"
          ...
          <div class="dang" @click="archiveProject(p)"><i class="ti ti-archive"></i> 归档项目</div>
        </div>
      </div>
      </div>
      <div v-if="projMenuId" @click="projMenuId = null"
           style="position:fixed;inset:0;z-index:var(--z-menu)"></div>
    </div>
    ...
    <div v-else v-show="!sectionCollapsed.sess" class="side-sess">
      ...
      <div v-for="grp in sessionGroups" :key="grp.key" v-show="grp.items.length" style="margin-bottom:5px">
    ...
    <!-- 添加项目对话框 -->
    <AddProjectModal v-if="addProjOpen"
                     @close="addProjOpen = false"
                     @created="onProjectCreated" />

    <!-- Ctrl+P 项目切换器（v5 §九 9.11） -->
    <div v-if="projectSwitcherOpen" class="proj-switcher-mask"
         @click="projectSwitcherOpen = false"></div>
    <div v-if="projectSwitcherOpen" class="proj-switcher">
      <input v-model="projectSwitcherQuery" class="ps-input"
             placeholder="输入项目名或路径过滤（Enter 选择，Esc 关闭）"
             autofocus @input="projectSwitcherHi = 0" />
      <div class="ps-list">
        <div v-for="(p, i) in projectSwitcherList" :key="p.id || 'none'"
             class="ps-item"
        ...
        <div v-if="!projectSwitcherList.length" class="ps-empty">
          没有匹配的项目
        </div>
      </div>
    </div>
...
.ws-hd .hd-toggle:hover, .sess-hd:hover { color: var(--fg); }
.side-workspace.ws-grow {
  flex: 0 1 auto; min-height: 0;
  overflow-y: auto; scrollbar-gutter: stable;
}
.side-sessions.sess-min { flex: 0 0 auto; }
.ws-add, .sess-add {
  cursor: pointer; padding: 2px 4px; border-radius: 4px;
}
.ws-add:hover, .sess-add:hover { background: var(--bg-hover, rgba(127,127,127,0.12)); color: var(--acctx); }
.sess-add { margin-left: auto; }
.ws-empty {
  padding: 12px; text-align: center;
  color: var(--muted); font-size: var(--fs-xs, 12px);
  line-height: 1.6;
}
.ws-project { margin-bottom: 1px; }
.ws-project-row {
  display: flex; align-items: center; gap: 8px;
  padding: 4px 12px; cursor: pointer; user-select: none;
  border-radius: 4px;
}
.ws-project-row:hover { background: var(--bg-hover, rgba(127,127,127,0.08)); }
.ws-project-row .ws-title {
  flex: 1; min-width: 0;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  font-size: var(--fs-sm, 13px);
}
.ws-project-row .ti-folder { color: var(--acctx); flex-shrink: 0; }
.ws-project-row .ws-missing { color: var(--muted); }
.ws-badge-miss {
...
.ws-count {
  font-size: var(--fs-xs, 12px); color: var(--muted);
  flex-shrink: 0;
}
.ws-dots {
  padding: 2px 4px; cursor: pointer; opacity: 0.5;
}
.ws-project-row:hover .ws-dots { opacity: 1; }
.ws-sessions { padding-left: 20px; }
.ws-sessions-empty {
  padding: 6px 12px; color: var(--muted);
  font-size: var(--fs-xs, 12px); font-style: italic;
}
.proj-menu {
  position: fixed;
  min-width: 168px;
...
.proj-menu > div:hover { background: var(--bg-hover, rgba(127,127,127,0.12)); }
.proj-menu > div.dang:hover { background: var(--dangtx-bg, rgba(200,0,0,0.1)); color: var(--dangtx); }
...
.proj-switcher .ps-list { max-height: 400px; overflow-y: auto; }
...
.proj-switcher .ps-item .ti { color: var(--acctx); }
.proj-switcher .ps-item .ps-title { font-weight: 500; }
