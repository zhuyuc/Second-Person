...
PARAM_SCHEMA: list[dict[str, Any]] = [
    ...
    {"key": "mood_fast_path_enabled", "type": "bool", "default": True,
     "effect": "next_turn", "group": "conversation", "order": 18,
     "label": "情绪快路径",
     "desc": "回复前用 Flash/词典判定本句用户情绪并注入尾部；关闭后仅用库内衰减态。"},
    {"key": "mood_fast_path_provider", "type": "enum",
     "options": ["flash", "lexicon"],
    ...
    {"key": "mood_fast_path_min_confidence", "type": "float", "min": 0.0, "max": 1.0,
     ...
     "label": "情绪脉冲最低置信度",
     "desc": "本轮脉冲置信度低于该值时忽略脉冲，仅用库内用户情绪态。"},
    {"key": "mood_fast_path_thinking", "type": "bool", "default": False,
     "effect": "next_turn", "group": "conversation", "order": 22,
     "label": "情绪快路径思考",
     "desc": "仅作用于 mood_fast 单次调用的 thinking_enabled；默认关闭。不会写入 chat/agent 等槽位默认配置。"},
    {"key": "mcp_tools_inject_mode", "type": "enum",
     "options": ["project_only", "always", "never"],
     "default": "project_only", "effect": "next_turn", "group": "conversation",
     "order": 17,
     "label": "MCP 工具注入",
     "desc": "外部连接器（如 GitHub）工具 schema 何时进入对话：project_only=仅项目会话；always=所有会话；never=从不。按会话/项目稳定轴切换，避免寒暄轮背负数千 token 的外部工具。"},
    ...
    # -- 本地文生图（ComfyUI + SDXL） --
    {"key": "image_gen_default_n", "type": "int", "min": 1, "max": 1,
     "default": 1, "effect": "immediate", "group": "other", "order": 20,
     "label": "文生图默认张数",
     "desc": "本地 SDXL 模式固定为 1 张，不可提高。"},
    {"key": "image_gen_max_n", "type": "int", "min": 1, "max": 1,
     "default": 1, "effect": "immediate", "group": "other", "order": 21,
     "label": "文生图单次上限",
     "desc": "本地模式强制上限 1，防止显存溢出。"},
    {"key": "image_gen_timeout_sec", "type": "int", "min": 30, "max": 600,
     "default": 180, "effect": "immediate", "group": "other", "order": 22,
     "label": "文生图单次请求超时（秒）",
     "desc": "云端生图 POST 读超时；下载阶段会自动重试直到成功或用户取消，"
             "不会因下载慢而把已成功的远端任务判失败。"},
    {"key": "image_gen_steps", "type": "int", "min": 10, "max": 50,
     "default": 24, "effect": "immediate", "group": "other", "order": 23,
     "label": "文生图采样步数",
     "desc": "仅本地 ComfyUI / SDXL 生效；云端忽略。"},
    {"key": "image_gen_prompt_max_chars", "type": "int", "min": 200, "max": 4000,
     "default": 1500, "effect": "immediate", "group": "other", "order": 24,
     "label": "文生图 prompt 最大字符",
     "desc": "送入生图引擎前截断 prompt，避免过长。"},
    {"key": "image_gen_refine_enabled", "type": "bool", "default": True,
     "effect": "immediate", "group": "other", "order": 25,
     "label": "文生图 prompt 自动润色",
     "desc": "仅本地 SDXL：出图前用轻量模型把中文/短句扩成英文；云端关闭润色。"},
    # -- 文生视频（本地 ComfyUI + Wan / 云端） --
    {"key": "video_gen_default_n", "type": "int", "min": 1, "max": 1,
     "default": 1, "effect": "immediate", "group": "other", "order": 30,
     "label": "文生视频默认条数",
     "desc": "一次固定 1 条，不可提高。"},
    {"key": "video_gen_max_n", "type": "int", "min": 1, "max": 1,
     "default": 1, "effect": "immediate", "group": "other", "order": 31,
     "label": "文生视频单次上限",
     "desc": "一次强制上限 1。"},
    {"key": "video_gen_timeout_sec", "type": "int", "min": 60, "max": 1800,
     "default": 600, "effect": "immediate", "group": "other", "order": 32,
     "label": "文生视频超时（秒）",
     "desc": "历史配置项；云端/本地出片现已跟到远端终态或用户取消，"
             "不再用本值把仍在运行的任务判为失败。"},
    {"key": "video_gen_steps", "type": "int", "min": 10, "max": 40,
     "default": 20, "effect": "immediate", "group": "other", "order": 33,
     "label": "文生视频采样步数",
     "desc": "仅本地 Wan 生效；云端忽略。"},
    {"key": "video_gen_default_duration_sec", "type": "int", "min": 2, "max": 4,
     "default": 3, "effect": "immediate", "group": "other", "order": 34,
     "label": "文生视频默认时长（秒）",
     "desc": "仅本地 Wan：允许 2–4 秒；云端默认 15 秒、最长 60 秒，不受此限制。"},
    {"key": "video_gen_max_duration_sec", "type": "int", "min": 2, "max": 4,
     "default": 4, "effect": "immediate", "group": "other", "order": 35,
     "label": "文生视频最长时长（秒）",
     "desc": "仅本地 Wan 硬上限 4 秒；云端最长 60 秒，不受此限制。"},
    {"key": "video_gen_fps", "type": "int", "min": 8, "max": 24,
     "default": 16, "effect": "immediate", "group": "other", "order": 36,
     "label": "文生视频帧率",
     "desc": "仅写入本地 Wan 工作流；不对 Agent 开放。"},
    {"key": "video_gen_prompt_max_chars", "type": "int", "min": 200, "max": 4000,
     "default": 1500, "effect": "immediate", "group": "other", "order": 37,
     "label": "文生视频 prompt 最大字符",
     "desc": "送入生视频引擎前截断 prompt。"},
    {"key": "video_gen_refine_enabled", "type": "bool", "default": True,
     "effect": "immediate", "group": "other", "order": 38,
     "label": "文生视频 prompt 自动润色",
     "desc": "仅本地 Wan：出片前扩成英文镜头描述；云端关闭润色。"},
    {"key": "video_gen_mutex_with_image_gen", "type": "bool", "default": True,
