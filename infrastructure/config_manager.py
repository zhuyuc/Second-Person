...
PARAM_SCHEMA: list[dict[str, Any]] = [
    ...
    # -- 对话参数 --
    {"key": "conversation_buffer_rounds", "type": "int", "min": 5, "max": 100,
     "default": 20, "effect": "next_session", "group": "conversation", "order": 1,
     "label": "对话缓冲区轮数",
     "desc": "单个会话最多保留多少轮完整上下文，超出后较早的内容会被压缩成摘要。"},
    {"key": "head_protected_messages", "type": "int", "min": 1, "max": 20,
     "default": 3, "effect": "next_session", "group": "conversation", "order": 2,
     "label": "开头保留条数",
     "desc": "压缩上下文时开头始终完整保留的消息条数（通常是背景设定），不被摘要。"},
    {"key": "compression_threshold_tokens", "type": "int", "min": 1000, "max": 2000000,
     "default": 80000, "effect": "next_session", "group": "conversation", "order": 3,
     "label": "压缩触发阈值（token）",
     "desc": "会话上下文累计超过该 token 数时，自动压缩早期消息以控制成本与延迟。"},
    # -- 情绪 --
