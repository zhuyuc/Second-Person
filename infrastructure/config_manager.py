...
PARAM_SCHEMA: list[dict[str, Any]] = [
    ...
    {"key": "memory_knowledge_min_score", "type": "float", "min": 0.0, "max": 100.0,
     "default": 55.0, "effect": "next_turn", "group": "memory", "order": 16,
     "label": "知识条目最低分",
     "desc": "外部文档/URL 导入产出的候选进入知识通道时的最低分门槛；低于该值会被拒绝，避免文档里对第三人的描述被当作个人记忆写入。"},
    {"key": "important_memory_decay_days", "type": "int", "min": 7, "max": 365,
     "default": 30, "effect": "immediate", "group": "memory", "order": 17,
     "label": "重要记忆衰减天数",
     "desc": "被标为重要的记忆连续该天数未被引用时，自动清除重要标记，避免早期误命中被永久放大权重。"},
    {"key": "memory_denial_suppress_window_minutes", "type": "int", "min": 5, "max": 240,
     "default": 30, "effect": "immediate", "group": "memory", "order": 18,
     "label": "否认信号抑制窗口（分钟）",
     "desc": "用户在会话中说\"不对/记错了\"时，会自动拒绝该会话最近该时间窗内入池的所有候选，避免下轮回顾链复原被否定的内容。"},
    {"key": "memory_distill_items_cap", "type": "int", "min": 1, "max": 32,
     "default": 8, "effect": "next_turn", "group": "memory", "order": 19,
     "label": "单次提炼候选数上限",
     "desc": "一次 distill 调用产出的候选数量硬上限，防止 LLM 一次性输出大量候选淹没候选池。"},
    {"key": "memory_domain_daily_cap", "type": "int", "min": 1, "max": 200,
     "default": 20, "effect": "immediate", "group": "memory", "order": 20,
     "label": "单 domain 每日入池上限",
     "desc": "同一个 domain 每天可以进入候选池的候选数上限；超额者不入池，仅记入操作日志。"},
    {"key": "freshness_boost_factor", "type": "float", "min": 1.0, "max": 2.0,
     "default": 1.15, "effect": "next_turn", "group": "memory", "order": 21,
     "label": "新鲜度加权系数",
     "desc": "近期创建的记忆在检索排序时得分上浮的倍率，避免用户成长后新观点被旧记忆压制。"},
    {"key": "freshness_boost_days", "type": "int", "min": 7, "max": 180,
     "default": 30, "effect": "next_turn", "group": "memory", "order": 22,
     "label": "新鲜度加权天数",
     "desc": "记忆创建后多少天内享受新鲜度加权。"},
    {"key": "json_repair_alert_threshold", "type": "int", "min": 1, "max": 100,
     "default": 5, "effect": "immediate", "group": "memory", "order": 22,
     "label": "JSON 修复告警阈值",
     "desc": "提取器连续 N 次 JSON 修复失败后推送系统通知。"},
    # -- 对话参数 --
