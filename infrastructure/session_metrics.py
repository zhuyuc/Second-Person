...
def record_step(db, *, turn_id: str, step: int, llm_ms: int,
    ...
    db.execute(
        ...
        "input_tokens,output_tokens,cache_read_tokens,cache_write_tokens,tool_ms,created_at) "
        "VALUES(?,?,?,?,?,?,?,?,?,?,?) "
        ...
        (turn_id, step, _num(llm_ms), ttft_ms, decode_ms, _num(input_tokens),
         ...
         _num(tool_ms), now_cst().isoformat(timespec="seconds")),
    )
...
def turn_metrics(db, turn_id: str) -> dict:
    rows = db.query_all(
        ...
        "cache_read_tokens,cache_write_tokens,tool_ms "
