...
from agent.turn_runtime_helpers import extract_web_citations
...
def test_extract_web_search_citations():
    ...
    cites = extract_web_citations("web_search", result)
    assert len(cites) == 2
...
def test_extract_ignores_other_tools():
    assert extract_web_citations("lookup", {"summary": "x"}) == []
